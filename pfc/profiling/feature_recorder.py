...
class FeatureRecorder:
    def __init__(
        ...
        split_batch_extra_keys: tuple[str, ...] = ("cfg_cat_batch_size", "batch_size"),
    ) -> None:
        ...
        self.split_batch_extra_keys = split_batch_extra_keys
    ...
    def _make_hook(self, module_name: str, module: nn.Module) -> Callable[[nn.Module, tuple[Any, ...], Any], None]:
        def hook(_module: nn.Module, _inputs: tuple[Any, ...], output: Any) -> None:
            ...
            if self.split_batch_dim0:
                record["split_batch_dim0"] = self._summarize_split_batch(tensor, module_name)
    ...
    def _previous_key(self, module_name: str) -> str:
        fields = []
        for field in self.previous_key_fields:
            if field == "module_name":
                fields.append(module_name)
            else:
                fields.append(str(self.context.get(field, "unknown")))
        return "::".join(fields)

    def _summarize_split_batch(self, tensor: torch.Tensor, module_name: str) -> dict[str, Any]:
        extra = self.context.get("extra") or {}
        batch_size = None
        batch_key = None
        for key in self.split_batch_extra_keys:
            value = extra.get(key)
        ...
        if batch_size is None:
            return {
                "skipped": True,
                "reason": f"missing positive integer batch size in extra keys {list(self.split_batch_extra_keys)}",
            }
        if tensor.ndim == 0 or tensor.shape[0] != batch_size * 2:
            actual = [] if tensor.ndim == 0 else [int(dim) for dim in tensor.shape]
            return {
                "skipped": True,
                "reason": f"expected dim0 == 2 * batch_size from extra[{batch_key}]",
