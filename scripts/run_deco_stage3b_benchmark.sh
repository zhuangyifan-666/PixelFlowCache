...
LOG="${ROOT}/logs/stage3b/deco_stage3b_benchmark_stdout.log"
exec > >(tee "${LOG}") 2>&1

echo "Stage 3B DeCo direct-velocity cache benchmark"
date -u +"UTC timestamp: %Y-%m-%dT%H:%M:%SZ"
if command -v nvidia-smi >/dev/null 2>&1; then
  if ! nvidia-smi; then
    echo "nvidia-smi failed; cannot run GPU Stage 3B benchmark safely."
    exit 1
  fi
else
  echo "nvidia-smi not found; cannot run GPU Stage 3B benchmark safely."
  exit 1
fi
...
export PFC_STAGE3B_SEEDS="${PFC_STAGE3B_SEEDS:-0}"
...
export PFC_STAGE3B_TIMING_REPEATS="${PFC_STAGE3B_TIMING_REPEATS:-2}"
export PFC_STAGE3B_WARMUP_RUNS="${PFC_STAGE3B_WARMUP_RUNS:-1}"
...
CMD=(python scripts/run_deco_stage3b_benchmark.py)
