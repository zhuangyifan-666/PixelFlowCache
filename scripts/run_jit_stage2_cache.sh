...
mkdir -p logs/stage2
LOG="${ROOT}/logs/stage2/jit_stage2_cache_stdout.log"
exec > >(tee "${LOG}") 2>&1

echo "Stage 2 JiT fixed-interval block cache"
...
export PFC_STAGE2_CACHE_INTERVAL="${PFC_STAGE2_CACHE_INTERVAL:-2}"
export PFC_STAGE2_CACHE_LAYERS="${PFC_STAGE2_CACHE_LAYERS:-middle}"
...
CMD=(python scripts/run_jit_stage2_cache.py)
