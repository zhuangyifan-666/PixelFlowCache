...
mkdir -p logs/stage1
LOG="${ROOT}/logs/stage1/deco_profile_stdout.log"
exec > >(tee "${LOG}") 2>&1

echo "Stage 1 DeCo profiling"
...
CMD=(python scripts/profile_deco_stage1.py)
echo "CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
echo "Final command: conda run --no-capture-output -n deco ${CMD[*]}"
conda run --no-capture-output -n deco "${CMD[@]}"
