...
LOG="${ROOT}/logs/stage3a/jit_stage3a_reduced_steps_stdout.log"
exec > >(tee "${LOG}") 2>&1

echo "Stage 3A JiT reduced-step baseline"
date -u +"UTC timestamp: %Y-%m-%dT%H:%M:%SZ"
if command -v nvidia-smi >/dev/null 2>&1; then
  if ! nvidia-smi; then
    echo "nvidia-smi failed; cannot run GPU Stage 3A reduced-step baseline safely."
    exit 1
  fi
else
  echo "nvidia-smi not found; cannot run GPU Stage 3A reduced-step baseline safely."
  exit 1
fi
...
export PFC_STAGE3A_TIMING_REPEATS="${PFC_STAGE3A_TIMING_REPEATS:-3}"
...
CMD=(python scripts/run_jit_stage3a_reduced_steps.py)
